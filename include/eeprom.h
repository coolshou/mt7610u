...
INT rtmp_ee_bin_write16(
	...
	IN USHORT 			data);
#endif //0
BOOLEAN rtmp_ee_load_from_bin(
	IN struct _RTMP_ADAPTER *pAd);

#if 0 //JB removed
BOOLEAN rtmp_ee_write_to_bin(
	IN struct _RTMP_ADAPTER *pAd);
#endif

#ifdef RT65xx
BOOLEAN rtmp_ee_write_to_prom(
	IN struct _RTMP_ADAPTER *pAd);
#endif /* RT65xx */
	
BOOLEAN Set_LoadEepromBufferFromBin_Proc(
	IN struct _RTMP_ADAPTER *pAd,
	IN PSTRING			arg);

BOOLEAN Set_EepromBufferWriteBack_Proc(
	IN struct _RTMP_ADAPTER *pAd,
	IN PSTRING			arg);
...
//JB removed BOOLEAN rtmp_get_default_bin_file_by_chip(struct _RTMP_ADAPTER *pAd, UINT32 ChipVersion, PSTRING *pBinFileName);
#endif /* __EEPROM_H__ */
