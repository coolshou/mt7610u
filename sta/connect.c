...
static VOID CntlIdleProc(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem);
static VOID CntlOidSsidProc(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM * Elem);
static VOID CntlMlmeRoamingProc(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM * Elem);
static VOID CntlWaitDisassocProc(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem);
static VOID CntlWaitJoinProc(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem);
static VOID CntlWaitStartProc(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem);
static VOID CntlWaitAuthProc(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem);
static VOID CntlWaitAuthProc2(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem);
static VOID CntlWaitAssocProc(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem);
static VOID CntlWaitReassocProc(RTMP_ADAPTER *pAd, MLME_QUEUE_ELEM *Elem);
static VOID IterateOnBssTab2(RTMP_ADAPTER *pAd);
static VOID JoinParmFill(RTMP_ADAPTER *pAd, MLME_JOIN_REQ_STRUCT *JoinReq,
	ULONG BssIdx);
static VOID StartParmFill(RTMP_ADAPTER *pAd,MLME_START_REQ_STRUCT *StartReq,
	CHAR Ssid[], UCHAR SsidLen);
static VOID AuthParmFill(RTMP_ADAPTER *pAd, MLME_AUTH_REQ_STRUCT *AuthReq,
	PUCHAR pAddr, USHORT Alg);
static VOID InitChannelRelatedValue(PRTMP_ADAPTER pAd);

static VOID MaintainBssTable(RTMP_ADAPTER *pAd, BSS_TABLE *Tab, 
	ULONG MaxRxTimeDiff, UCHAR MaxSameRxTimeCount);

#if 0 //JB removed
static UCHAR CipherSuiteWpaNoneTkip[] = {
...
static UCHAR CipherSuiteWpaNoneTkipLen =
	(sizeof (CipherSuiteWpaNoneTkip) / sizeof (UCHAR));

static UCHAR CipherSuiteWpaNoneAes[] = {
	0x00, 0x50, 0xf2, 0x01,	/* oui */
	0x01, 0x00,		/* Version */
...
static UCHAR CipherSuiteWpaNoneAesLen =
...
/*
	...
	==========================================================================
*/
VOID MlmeCntlInit(PRTMP_ADAPTER pAd, STATE_MACHINE *S, STATE_MACHINE_FUNC Trans[])
{
...
/*
	...
	==========================================================================
*/
VOID MlmeCntlMachinePerformAction(PRTMP_ADAPTER pAd, STATE_MACHINE *S, 
...
/*
	...
	==========================================================================
*/
static VOID CntlIdleProc(PRTMP_ADAPTER pAd, MLME_QUEUE_ELEM *Elem)
{
...
VOID CntlOidScanProc(PRTMP_ADAPTER pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	if (OPSTATUS_TEST_FLAG(pAd, fOP_STATUS_MEDIA_STATE_CONNECTED)) {
		BssIdx = BssSsidTableSearch(&pAd->ScanTab, pAd->CommonCfg.Bssid,
...
/*
	...
	==========================================================================
*/
static VOID CntlOidSsidProc(PRTMP_ADAPTER pAd, MLME_QUEUE_ELEM *Elem)
{
	...
	*/
	...
	if (pAd->MlmeAux.SsidLen == MAX_LEN_OF_SSID)
		...
		DBGPRINT(RT_DEBUG_TRACE, ("%s: (%d)SSID - %s\n",
				__FUNCTION__,pAd->MlmeAux.SsidLen, pAd->MlmeAux.Ssid));
		...
		*/
		...
		if (((wdev->AuthMode == Ndis802_11AuthModeWPA) ||
			...
			*/
			...
			RtmpOSWrielessEventSend(pAd->net_dev, RT_WLAN_EVENT_CGIWAP, 
					-1, &pAd->MlmeAux.Bssid[0], NULL, 0);
#endif
		}
		...
		*/
		...
		if ((pAd->MlmeAux.SsidBssTab.BssNr == 0) && 
			...
			NdisZeroMemory(pAd->cfg80211_ctrl.Cfg_pending_Ssid, 
					MAX_LEN_OF_SSID + 1);
			NdisMoveMemory(pAd->cfg80211_ctrl.Cfg_pending_Ssid, 
...
/*
	...
	==========================================================================
*/
VOID CntlOidRTBssidProc(PRTMP_ADAPTER pAd, MLME_QUEUE_ELEM *Elem)
{
	ULONG BssIdx;
	...
	if (BssIdx != BSS_NOT_FOUND) {
		...
		*/
		if (SSID_EQUAL(pAd->MlmeAux.Ssid, pAd->MlmeAux.SsidLen, 
			pInBss->Ssid, pInBss->SsidLen) == FALSE) {
...
/* */
/* IRQL = DISPATCH_LEVEL */
static VOID CntlMlmeRoamingProc(PRTMP_ADAPTER pAd, MLME_QUEUE_ELEM *Elem)
{
...
/*
	...
	==========================================================================
*/
static VOID CntlWaitDisassocProc(PRTMP_ADAPTER pAd, MLME_QUEUE_ELEM *Elem)
{
	MLME_START_REQ_STRUCT StartReq;
...
/*
	...
	==========================================================================
*/
static VOID CntlWaitJoinProc(PRTMP_ADAPTER pAd, MLME_QUEUE_ELEM *Elem)
{
	USHORT Reason;
...
/*
	...
	==========================================================================
*/
static VOID CntlWaitStartProc(PRTMP_ADAPTER pAd, MLME_QUEUE_ELEM *Elem)
{
	USHORT Result;
...
/*
	...
	==========================================================================
*/
static VOID CntlWaitAuthProc(PRTMP_ADAPTER pAd, MLME_QUEUE_ELEM *Elem)
{
	USHORT Reason;
...
/*
	...
	==========================================================================
*/
static VOID CntlWaitAuthProc2(PRTMP_ADAPTER pAd, MLME_QUEUE_ELEM *Elem)
{
	USHORT Reason;
	MLME_ASSOC_REQ_STRUCT AssocReq;
	MLME_AUTH_REQ_STRUCT AuthReq;
...
/*
	...
	==========================================================================
*/
static VOID CntlWaitAssocProc(PRTMP_ADAPTER pAd, MLME_QUEUE_ELEM *Elem)
{
	USHORT Reason;
...
/*
	...
	==========================================================================
*/
static VOID CntlWaitReassocProc(PRTMP_ADAPTER pAd, MLME_QUEUE_ELEM *Elem)
{
	USHORT Result;
...
VOID LinkUp(RTMP_ADAPTER *pAd, UCHAR BssType)
{
	...
	*/
	...
	DBGPRINT(RT_DEBUG_TRACE, ("!!! LINK UP !!! (%s)\n",
...
*/
	...
	if (BssType == BSS_ADHOC) {
		...
		if (wdev->WepStatus == Ndis802_11WEPEnabled) {
			...
			/* Update WCID attribute table and IVEIV table for this group key table */
			RTMPSetWcidSecurityInfo(pAd, BSS0, 0,
...
/*
	...
	==========================================================================
*/
VOID LinkDown(PRTMP_ADAPTER pAd, BOOLEAN IsReqFromAP)
{
	UCHAR i;
	struct wifi_dev *wdev = &pAd->StaCfg.wdev;
...
/*
	...
	==========================================================================
*/
static VOID JoinParmFill(PRTMP_ADAPTER pAd, MLME_JOIN_REQ_STRUCT *JoinReq, 
	ULONG BssIdx)
{
...
/*
	...
	==========================================================================
*/
static VOID StartParmFill(PRTMP_ADAPTER pAd, MLME_START_REQ_STRUCT *StartReq,
	CHAR Ssid[], UCHAR SsidLen)
{
...
/*
	...
	==========================================================================
*/
static VOID AuthParmFill(PRTMP_ADAPTER pAd, MLME_AUTH_REQ_STRUCT *AuthReq,
	PUCHAR pAddr, USHORT Alg)
{
...
VOID MlmeCntlConfirm(PRTMP_ADAPTER pAd, ULONG MsgType, USHORT Msg)
{
...
/*
	...
	==========================================================================
*/
ULONG MakeIbssBeacon(PRTMP_ADAPTER pAd)
{
	...
	CapabilityInfo = CAP_GENERATE(0, 1, Privacy,
...
static VOID InitChannelRelatedValue(RTMP_ADAPTER *pAd)
{
	UCHAR rf_channel;
	UINT8 rf_bw, ext_ch;
...
/* IRQL = DISPATCH_LEVEL */
static VOID MaintainBssTable(PRTMP_ADAPTER pAd, BSS_TABLE *Tab,
	ULONG MaxRxTimeDiff, UCHAR MaxSameRxTimeCount)
{
	UCHAR i, j;
	...
	USHORT ifIndex, BOOLEAN BandWidth, UCHAR PriCh, UCHAR ExtraCh)
{
	...
	*pBwFallBack = 0;
	...
	DBGPRINT(RT_DEBUG_TRACE, 
			("%s():CentralChannel=%d, Channel=%d, ChannelWidth=%d\n",
	...
	DBGPRINT(RT_DEBUG_OFF, 
			...
			__FUNCTION__, rf_bw, rf_channel, pAd->CommonCfg.vht_bw, 
			pAd->CommonCfg.Channel, pAd->CommonCfg.vht_cent_ch));
#endif /* DOT11_VHT_AC */
